import { lookupAllByPassphrase } from '@/lib/services/protocol';
...
export async function POST(request: Request) {
  ...
  try {
    const protocols = await lookupAllByPassphrase(body.passphrase.trim().toLowerCase());

    if (protocols.length === 0) {
      // Add a small delay to prevent timing attacks
      await new Promise((resolve) => setTimeout(resolve, 500));
      return Response.json({ found: false, protocols: [] });
    }
    ...
    console.error('Lookup-all error:', error);
    return Response.json(
